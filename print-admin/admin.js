$(document).ready(function() {
	...
	$("#upload_frame").on("change", function() {
		if ($(this).val() != "") {
			...
			formData.append('upload_frame', $('#upload_frame')[0].files[0]);
			$.ajax({
				url: 'upload.php',
				data: formData,
				type: 'POST',
		...
		$("#myModal").on('shown.bs.modal', function() {
			$.ajax({
				...
				success: function(result) {
					...
					$("#image_content").html("<img src='" + result.img_path + "' height='200'/>");
	...
	$(".edit-translation").on("click", function() {
		...
		$("#myModal").on('shown.bs.modal', function() {
			$.ajax({
				...
				success: function(result) {
					var result = JSON.parse(result);
					$("#translation_id").val(result.id);
					$("#translation_lang_en").val(result.lang_en);
					$("#translation_lang_hb").val(result.lang_hb);
					$("#translation_status").val(result.active);
					$("#frame_status option").each(function(index) {
						if ($(this).attr("value") == result.active) {
							$(this).attr("selected", true);
							return false;
